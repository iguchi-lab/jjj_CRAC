...
def calc_heating_load(region, sol_region, A_A, A_MR, A_OR, Q, mu_H, mu_C, NV_MR, NV_OR, TS, r_A_ufvnt, HEX, underfloor_insulation, mode_H, mode_C,
    ...
    :param mode_C: 冷房方式
    :type mode_C: str
    :param spec_MR: 主たる居室の暖房機器の仕様
    :type spec_MR: dict
    :param spec_OR: その他の居室の暖房機器の仕様
    :type spec_OR: dict
    ...
    :type SHC: dict
    :return: (暖房区画i=1-5それぞれの暖房負荷, 標準住戸の暖冷房区画iの負荷補正前の暖房負荷 (MJ/h))
...
def calc_Q_UT_H_MR_d_t(region, A_A, A_MR, A_OR, spec_MR, spec_OR, spec_HS, mode_MR, mode_OR, CG, L_T_H_d_t):
    ...
    :return: 住戸全体を連続的に暖房する方式における1時間当たりの主たる居室の暖房設備の未処理暖房負荷(MJ/h)
    :rtype: ndarray
    """
...
def calc_Q_UT_H_OR_d_t(region, A_A, A_MR, A_OR, spec_MR, spec_OR, spec_HS, mode_MR, mode_OR, CG, L_T_H_d_t):
    ...
    :param L_T_H_d_t: 暖房区画の暖房負荷
    :type L_T_H_d_t: ndarray
    :return: 住戸全体を連続的に暖房する方式における1時間当たりのその他の居室の暖房設備の未処理暖房負荷(MJ/h)
    :rtype: ndarray
    """
    # その他の居室がない場合
    if A_OR == 0:
...
def calc_Q_UT_H_d_t(i, device, A_A, A_MR, A_OR, region, mode, L_H_d_t):
    ...
    :param region: 省エネルギー地域区分
    :type region: int
    :param mode: 運転方法 (連続運転|間歇運転)
    :type mode: str
    :param L_H_d_t: 暖冷房区画݅の１時間当たりの暖房負荷(MJ/h)
    :type L_H_d_t: ndarray
    :return: ：1時間当たりの暖房設備機器等の未処理暖房負荷(MJ/h)
    :rtype ndarray:
    """
    # 床面積
    A_HCZ = calc_A_HCZ_i(i, A_A, A_MR, A_OR)
...
def get_Q_UT_H_d_t_i(Q_T_H_d_t_i, L_H_d_t_i):
    ...
    :param Q_T_H_d_t_i: 暖冷房区画iに設置された1時間当たりの処理暖房負荷 (MJ/h)
    :type Q_T_H_d_t_i: ndarray
    :param L_H_d_t_i:日付dの時刻tにおける暖冷房区画iの1時間当たりの暖房負荷（MJ/h）
    :type L_H_d_t_i: ndarray
    :return:
    :rtype:
...
def get_E_E_H_d_t(region, sol_region, A_A, A_MR, A_OR, A_env, mu_H, mu_C, Q,
    ...
    :param L_T_CS_d_t_i: 冷房区画i=1-5それぞれの冷房顕熱負荷
    :type L_T_CS_d_t_i: ndarray
    :param L_T_CL_d_t_i: 冷房区画i=1-5それぞれの冷房潜熱負荷
    :type L_T_CL_d_t_i: ndarray
    :return: 暖房設備の消費電力量（kWh/h）
    :rtype: ndarray
    """
    if region == 8:
        return np.zeros(24 * 365)
...
def get_E_E_aux_ass_d_t(SHC, heating_flag_d, region, sol_region):
    ...
    :param sol_region:
    :type sol_region:
    :return: 空気集熱式太陽熱利用設備の補機の消費電力量のうちの暖房設備への付加分（kWh/h）
    :rtype: ndarray
    """
...
def is_hotwaterheatingonly(H_MR, H_OR):
    ...
    :type H_OR: dict
    :return: 居室のみを暖房する方式でかつ主たる居室とその他の居室ともに温水暖房を設置するかどうか
    :rtype: bool
    """
...
def calc_E_E_H_hs_MR_d_t(region, A_A, A_MR, A_OR, mode_MR, mode_OR, spec_MR, spec_OR, spec_HS, L_T_H_d_t_i, L_CS_x_t_i, L_CL_x_t_i, CG=None):
    ...
    :param L_T_H_d_t_i: 暖房区画i=1-5それぞれの暖房負荷
    :type L_T_H_d_t_i: ndarray
    :param L_CS_x_t_i: 冷房区画i=1-5それぞれの冷房顕熱負荷
    :type L_CS_x_t_i: ndarray
    :param L_CL_x_t_i: 冷房区画i=1-5それぞれの冷房潜熱負荷
    :type L_CL_x_t_i: ndarray
    :param CG: コージェネレーションの機器
    :type CG: dict
    :return: 居室のみを暖房する方式における主たる居室の暖房設備機器の消費電力量（kWh/h）
    :rtype: ndarray
    """
    if spec_MR['type'] in ['温水暖房用パネルラジエーター', '温水暖房用床暖房', '温水暖房用ファンコンベクター']:
...
def get_E_E_H_hs_OR_d_t(region, A_A, A_MR, A_OR, mode_MR, mode_OR, spec_MR, spec_OR, spec_HS, L_T_H_d_t_i, L_CS_x_t_i, L_CL_x_t_i, CG=None):
    ...
    :param CG: コージェネレーションの機器
    :type CG: dict
    :return: 居室のみを暖房する方式におけるその他の居室の暖房設備機器の消費電力量（kWh/h）
    :rtype: ndarray
    """
    # その他の居室がない場合
    if spec_OR is None:
...
def calc_L_H_d_t(region, sol_region, A_A, A_MR, A_OR, mode_H, mode_C, H_MR, H_OR, mode_MR, mode_OR, Q, mu_H, mu_C, NV_MR, NV_OR, TS, r_A_ufvnt, HEX,
    ...
    :param SHC: 集熱式太陽熱利用設備の仕様
    :type SHC: dict
    :param underfloor_insulation:
    :type underfloor_insulation:
    :param normalize: 正規化の有無
    :type normalize: bool
    :return: 暖冷房区画の１時間当たりの暖房負荷
    :rtype: ndarray
    """
    # 温水暖房
...
def calc_E_G_H_hs_MR_d_t(region, A_A, A_MR, A_OR, mode_MR, mode_OR, spec_MR, spec_OR, spec_HS, L_T_H_d_t, CG):
    ...
    :param spec_HS: 温水暖房機の仕様
    :type spec_HS: dict
    :param L_T_H_d_t: 暖房負荷
    :type L_T_H_d_t: ndarray
    :param CG: コージェネレーションの機器
    :type CG: dict
    :return: 居室のみを暖房する方式における主たる居室に設置された暖房設備機器のガス消費量（MJ/h）
    :rtype: ndarray
    """
    if spec_MR['type'] in ['温水暖房用パネルラジエーター', '温水暖房用床暖房', '温水暖房用ファンコンベクター']:
...
def calc_E_G_H_hs_OR_d_t(region, A_A, A_MR, A_OR, mode_MR, mode_OR, spec_MR, spec_OR, spec_HS, L_T_H_d_t, CG=None):
    ...
    :param CG: コージェネレーションの機器
    :type CG: dict
    :return: 居室のみを暖房する方式におけるその他の居室に設置された暖房設備機器のガス消費量（MJ/h）
    :rtype: ndarray
    """
    # その他の居室がない場合
    if spec_OR is None:
...
def calc_E_K_H_hs_OR_d_t(region, A_A, A_MR, A_OR, spec_MR, spec_OR, spec_HS, mode_MR, mode_OR, L_T_H_d_t, CG):
    ...
    :param mode_OR: その他の居室の運転方法 (連続運転|間歇運転)
    :type mode_OR: str
    :param L_T_H_d_t: 暖房区画i=1-5それぞれの暖房負荷
    :type L_T_H_d_t: ndarray
    :param CG: コージェネレーションの機器
    :type CG: dict
    :return: 居室のみを暖房する方式におけるその他の居室に設置された暖房設備機器のガス消費量(MJ/h)
...
def get_virtual_heating_devices(region, H_MR, H_OR):
    ...
    :param H_OR: その他の居室の暖房機器の仕様
    :type H_OR: dict
    :return: 実質的な暖房機器の仕様(主たる居室, その他の居室)
...
def calc_E_K_d_t(i, device, A_A, A_MR, A_OR, L_H_d_t):
    ...
    :param L_H_d_t: 暖冷房区画݅の１時間当たりの暖房負荷(MJ/h)
    :type L_H_d_t: ndarray
    :return: 暖房区画1つと1つの暖房設備機器によって消費されるガス消費量(MJ/h)
    :rtype: ndarray
    """
    # 床面積
    A_HCZ = calc_A_HCZ_i(i, A_A, A_MR, A_OR)
...
def calc_E_M_H_d_t(region, A_A, A_MR, A_OR, H_A, spec_MR, spec_OR, spec_HS, L_T_H_d_t_i):
    ...
    :param L_T_H_d_t_i: 暖房区画i=1-5それぞれの暖房負荷
    :type L_T_H_d_t_i: ndarray
    :return: 暖房設備のその他の燃料による一次エネルギー消費量（MJ/h）
    :rtype: ndarray
    """
...
def calc_E_M_hs_d_t(region, A_A, A_MR, A_OR, H_A, spec_MR, spec_OR, spec_HS, L_H_d_t):
    ...
    :param L_H_d_t: 暖冷房区画݅の１時間当たりの暖房負荷(MJ/h)
    :type L_H_d_t: ndarray
    :return: 暖房設備機器等のその他の燃料による一次エネルギー消費量（MJ/h）
    :rtype: ndarray
    """
    if H_A is not None:
        # (7d)
...
def calc_E_G_H_hs_A_d_t():
    ...
    :return: ：住戸全体を連続的に暖房する方式における暖房設備機器のガス消費量（MJ/h）
    :rtype: ndarray
    """
...
def calc_E_K_H_hs_A_d_t(**args):
    ...
    :return: 住戸全体を連続的に暖房する方式における暖房設備機器の灯油消費量（MJ/h）
    :rtype: ndarray
    """
...
def calc_E_E_H_hs_MROR_d_t(region, A_A, A_MR, A_OR, H_MR, H_OR, H_HS, CG, L_T_H_d_t_i, L_CS_x_t_i, L_CL_x_t_i):
    ...
    :param L_CL_x_t_i: 冷房区画i=1-5それぞれの冷房潜熱負荷
    :type L_CL_x_t_i: ndarray
    :return: 居室のみを暖房する方式における主たる居室及びその他の居室に設置された暖房設備機器の消費電力量（kWh/h）
    :rtype: ndarray
    """
    # 暖房方式及び運転方法の区分
...
def calc_E_G_H_hs_MROR_d_t(mode_MR, mode_OR, spec_HS, spec_MR, spec_OR, L_T_H_d_t_i, CG, A_A, A_MR, A_OR, region):
    ...
    :param region: 省エネルギー地域区分
    :type region: int
    :return: 居室のみを暖房する方式における主たる居室及びその他の居室に設置された暖房設備機器のガス消費量（MJ/h）
    :rtype: ndarray
    """
...
def calc_E_K_H_hs_MROR_d_t(spec_HS, spec_MR, spec_OR, CG, A_A, A_MR, A_OR, region, mode_MR, mode_OR, L_T_H_d_t_i):
    ...
    :param L_T_H_d_t_i: 暖房区画i=1-5それぞれの暖房負荷
    :type L_T_H_d_t_i: ndarray
    :return: 居室のみを暖房する方式における主たる居室及びその他の居室に設置された暖房設備機器の灯油消費量（MJ/h）
    :rtype: ndarray
    """
...
def calc_E_M_H_hs_MROR_d_t(region, H_HS):
    ...
    :param H_HS: 温水暖房機の仕様
    :type H_HS: dict
    :return: 居室のみを暖房する方式における主たる居室及びその他の居室に設置された暖房設備機器のその他の燃料による一次エネルギー消費量（MJ/h）
    :rtype: ndarray
    """
    # 実質的な温水暖房熱源の取得
...
def calc_E_M_H_hs_MR_d_t(A_A, A_MR, A_OR, spec_MR, spec_HS, L_H_d_t):
    ...
    :param L_H_d_t: 暖冷房区画݅の１時間当たりの暖房負荷(MJ/h)
    :type L_H_d_t: ndarray
    :return: 居室のみを暖房する方式における主たる居室に設置された暖房設備機器のその他の燃料による一次エネルギー消費量（MJ/h）
    :rtype: ndarray
    """
    if spec_MR['type'] in ['温水暖房用パネルラジエーター', '温水暖房用床暖房', '温水暖房用ファンコンベクター']:
        return hwh.calc_E_M_H_d_t(spec_HS)
...
def calc_E_M_H_hs_OR_d_t(A_A, A_MR, A_OR, spec_OR, spec_HS, L_H_d_t):
    ...
    :param L_H_d_t: 暖冷房区画݅の１時間当たりの暖房負荷(MJ/h)
    :type L_H_d_t: ndarray
    :return: 居室のみを暖房する方式におけるその他の居室に設置された暖房設備機器のその他の燃料による一次エネルギー消費量（MJ/h）
    :rtype: ndarray
    """
    # その他の居室がない場合
    if spec_OR is None:
...
def calc_E_M_d_t(i, device, A_A, A_MR, A_OR, L_H_d_t):
    ...
    :param L_H_d_t: 暖冷房区画݅の１時間当たりの暖房負荷(MJ/h)
    :type L_H_d_t: ndarray
    :return: 暖房区画1つと1つの暖房設備機器によって消費されるその他の一次エネルギー消費量
    :rtype: ndarray
    """
    # 床面積
    A_HCZ = calc_A_HCZ_i(i, A_A, A_MR, A_OR)
...
def get_table_3():
    ...
    :return: 表 3 未処理暖房負荷を未処理暖房負荷の設計一次エネルギー消費量相当値に換算するための係数
    :rtype: list
    """
    table_3 = [
        1.61,
...
def get_alpha_UT_H_A(region):
    ...
    :param region: 省エネルギー地域区分
    :type region: int
    :return: 未処理暖房負荷を未処理暖房負荷の設計一次エネルギー消費量相当値に換算するための係数
    :rtype: float
    """
...
def get_table_4():
    ...
    :return: 表 4 未処理暖房負荷を未処理暖房負荷の設計一次エネルギー消費量相当値に換算するための係数
    :rtype: list
    """
    table_4 = [
        (1.59, 1.21, 1.59, 1.22),
...
def get_alpha_UT_H_MR(region, mode_H_MR):
    ...
    :param region: 省エネルギー地域区分
    :type region: int
    :param mode_H_MR: 主たる居室の運転方法 (連続運転|間歇運転)
    :type mode_H_MR: str
    :return: 居室のみを暖房する方式における主たる居室に設置された暖房設備の未処理暖房負荷を未処理暖房負荷の設計一次エネルギー消費量相当値に換算するための係数
    :rtype: float
    """
...
def get_alpha_UT_H_OR(region, mode_H_OR):
    ...
    :param region: 省エネルギー地域区分
    :type region: int
    :param mode_H_OR: 主たる居室の運転方法 (連続運転|間歇運転)
    :type mode_H_OR: str
    :return: 居室のみを暖房する方式におけるその他の居室に設置された暖房設備の未処理暖房負荷を未処理暖房負荷の設計一次エネルギー消費量相当値に換算するための係数
    :rtype: float
    """
...
def calc_cooling_load(region, A_A, A_MR, A_OR, Q, mu_H, mu_C, NV_MR, NV_OR, r_A_ufvnt, underfloor_insulation, mode_C, mode_H, mode_MR, mode_OR, TS, HEX):
    ...
    :param HEX: 熱交換器型設備仕様辞書
    :type HEX: dict
    :return: 冷房負荷(冷房顕熱負荷, 冷房潜熱負荷)(MJ/h)
    :rtype: tuple(ndarray, ndarray)
    """
...
def calc_Q_UT_CS_A_d_t(A_A, A_MR, A_OR, A_env, mu_H, mu_C, q_hs_rtd_H, q_hs_rtd_C, V_hs_dsgn_H, V_hs_dsgn_C, Q,
    ...
    :param L_CL_d_t: 冷房区画の冷房潜熱負荷
    :type L_CL_d_t: ndarray
    :return: 住戸全体を連続的に冷房する方式における冷房設備の未処理冷房顕熱負荷（MJ/h）
    :rtype: ndarray
    """
...
def calc_Q_UT_CL_A_d_t(A_A, A_MR, A_OR, A_env, mu_H, mu_C, q_hs_rtd_H, q_hs_rtd_C, V_hs_dsgn_H, V_hs_dsgn_C, Q,
    ...
    :param L_CL_d_t: 冷房区画の冷房潜熱負荷
    :type L_CL_d_t: ndarray
    :return: 住戸全体を連続的に冷房する方式における冷房設備の未処理冷房潜熱負荷（MJ/h）
    :rtype: ndarray
    """
...
def calc_Q_T_CS_MR_d_t(**args):
    ...
    :return: 居室のみを冷房する方式における主たる居室に設置された冷房設備の処理冷房顕熱負荷
    :rtype: ndarray
    """
...
def calc_Q_T_CL_MR_d_t(**args):
    ...
    :return: 居室のみを冷房する方式における主たる居室に設置された冷房設備の処理冷房潜熱負荷
    :rtype: ndarray
    """
...
def calc_Q_UT_CS_MR_d_t(**args):
    ...
    :return: 居室のみを冷房する方式における主たる居室に設置された冷房設備の未処理冷房顕熱負荷
    :rtype: ndarray
    """
...
def calc_Q_UT_CL_MR_d_t(**args):
    ...
    :return: 居室のみを冷房する方式における主たる居室に設置された冷房設備の未処理冷房潜熱負荷
    :rtype: ndarray
    """
...
def calc_Q_T_CS_OR_d_t(**args):
    ...
    :return: 居室のみを冷房する方式におけるその他の居室に設置された冷房設備の処理冷房顕熱負荷
    :rtype: ndarray
    """
...
def calc_Q_T_CL_OR_d_t(**args):
    ...
    :return: 居室のみを冷房する方式におけるその他の居室に設置された冷房設備の処理冷房潜熱負荷
    :rtype: ndarray
    """
...
def calc_Q_UT_CS_OR_d_t(**args):
    ...
    :return: 居室のみを冷房する方式におけるその他の居室に設置された冷房設備の未処理冷房顕熱負荷
    :rtype: ndarray
    """
...
def calc_Q_UT_CL_OR_d_t(**args):
    ...
    :return: 居室のみを冷房する方式におけるその他の居室に設置された冷房設備の未処理冷房潜熱負荷
    :rtype: ndarray
    """
...
def calc_E_M_C_d_t(**args):
    ...
    :return: 冷房設備のその他の燃料による一次エネルギー消費量
    :rtype: ndarray
    """
...
def get_E_M_C_hs_d_t():
    ...
    :return: 冷房設備機器のその他の燃料による一次エネルギー消費量（MJ/h）
    :rtype: ndarray
    """
...
def calc_E_E_C_hs_MR_d_t(region, A_A, A_MR, A_OR, C_MR, L_CS_d_t, L_CL_d_t):
    ...
    :param L_CL_d_t: 冷房区画の冷房潜熱負荷
    :type L_CL_d_t: ndarray
    :return: 居室のみを冷房する方式における主たる居室に設置された冷房設備機器の消費電力（kWh/h）
    :rtype: ndarray
    """
    spec_MR = C_MR
...
def calc_E_G_C_hs_MR_d_t():
    ...
    :return: 居室のみを冷房する方式における主たる居室に設置された冷房設備機器のガス消費量（MJ/h）
    :rtype: ndarray
    """
...
def calc_E_K_C_hs_MR_d_t():
    ...
    :return: 居室のみを冷房する方式における主たる居室に設置された冷房設備機器の灯油消費量（MJ/h）
    :rtype: ndarray
    """
...
def calc_E_M_C_hs_MR_d_t():
    ...
    :return: 居室のみを冷房する方式における主たる居室に設置された冷房設備機器のその他の燃料による一次エネルギー消費量（MJ/h）
    :rtype: ndarray
    """
...
def calc_E_E_C_hs_OR_d_t(region, A_A, A_MR, A_OR, C_OR, L_CS_d_t, L_CL_d_t):
    ...
    :param L_CL_d_t: 冷房区画の冷房潜熱負荷
    :type L_CL_d_t: ndarray
    :return: 居室のみを冷房する方式におけるその他の居室に設置された冷房設備機器の消費電力（kWh/h）
    :rtype: ndarray
    """
    # その他の居室がない場合
...
def calc_E_E_C_hs_d_t_i(i, device, region, A_A, A_MR, A_OR, L_CS_d_t, L_CL_d_t):
    ...
    :param L_CL_d_t: 冷房区画の冷房潜熱負荷
    :type L_CL_d_t: ndarray
    :return: 暖冷房区画𝑖に設置された冷房設備機器の消費電力量（kWh/h）
    :rtype: ndarray
    """
    if device['type'] == 'ルームエアコンディショナー':
...
def calc_E_G_C_hs_OR_d_t():
    ...
    :return: 居室のみを冷房する方式におけるその他の居室に設置された冷房設備機器のガス消費量（MJ/h）
    :rtype: ndarray
    """
...
def calc_E_K_C_hs_OR_d_t():
    ...
    :return: 居室のみを冷房する方式におけるその他の居室に設置された冷房設備機器の灯油消費量（MJ/h）
    :rtype: ndarray
    """
...
def calc_E_M_C_hs_OR_d_t():
    ...
    :return: 居室のみを冷房する方式におけるその他の居室に設置された冷房設備機器のその他の燃料による一次エネルギー消費量（MJ/h）
    :rtype: ndarray
    """
